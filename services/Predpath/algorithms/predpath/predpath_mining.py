...
def predict(G, triples, vec, model):
	...
	# Path extraction
	print '=> Path extraction.. (this can take a whiles)'
	t1 = time()
	features, pos_features, neg_features, measurements = extract_paths(G, triples, y)
	print 'P: +:{}, -:{}, unique tot:{}'.format(len(pos_features), len(neg_features), len(features))
...
def find_best_model(X, y, scoring='roc_auc', cv=10):
	...
	grid_search.fit(X, y)
	#print("<<<<<<<<<<trying to use the model on our test case>>>>>>>>>>>")
	#X_test = tvect.transform([["392035", "599", "2115741"]])
	#y_pred_acc = grid_search.predict(X_test)
	#print("The result is %s" % (y_pred_acc))
	best = {
